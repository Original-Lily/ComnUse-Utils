...
Function copy-Files-To-Ram-Drive() {
    if (-Not ((Test-Path "$($SYSTEM_DRIVE)\DiskSn.bat") -And (Test-Path "$($SYSTEM_DRIVE)\DiskSn.ps1"))) {
        Write-Host ""
        Write-Host "RAM Drive missing Disk San files. Copying files necessary to be able to remove USB..." -ForegroundColor Green
        $JUNK | Out-File .\_._
        Copy-Item -Path .\_._ -Destination "$($SYSTEM_DRIVE)\_._"
        Copy-Item -Path .\DiskSn.bat -Destination "$($SYSTEM_DRIVE)\DiskSn.bat"
        Copy-Item -Path .\DiskSn.ps1 -Destination "$($SYSTEM_DRIVE)\DiskSn.ps1"
    }
    else {
        return #Files already exist, do exit function
    }
    Set-Location $ENV:SystemDrive
    Write-Host "Relaunching Disk Sanitizer from Ram Drive." -ForegroundColor Green
    Start-Sleep -Seconds 3
    Powershell.exe -File .\DiskSn.ps1
    Exit
}
